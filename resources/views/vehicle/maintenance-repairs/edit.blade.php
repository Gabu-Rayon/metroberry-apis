<form action="{{ route('maintenance.repair.edit', $repair->id) }}" method="POST" class="needs-validation modal-content" novalidate="novalidate" enctype="multipart/form-data" id="editRepairForm">
    @csrf
    @method('PUT')
        ...
        <div class="row">
            <div class="col-md-12 col-lg-6">
                    ...
                    <div class="col-sm-7">
                        <select name="vehicle_id" class="form-control" id="vehicle_id" required>
                            ...
                            @foreach($vehicles as $vehicle)
                                <option value="{{ $vehicle->id }}" {{ $vehicle->id == $repair->vehicle->id ? 'selected' : '' }}>
                    ...
                    <div class="col-sm-7">
                        <select name="part_id" class="form-control" id="part_id" required>
                            ...
                            @foreach($parts as $part)
                                <option value="{{ $part->id }}" {{ $part->id == $repair->part->id?'selected' : '' }} data-price="{{ $part->price }}">
                                    {{ $part->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row my-2">
                    ...
                    <div class="col-sm-7">
                        <input type="number" name="cost" class="form-control" id="cost" step="0.01" min="0" value="{{ $repair->repair_cost }}" />
                    </div>
                </div>
                ...
                <div class="form-group row my-2">
                    ...
                    <div class="col-sm-7">
                        <input type="number" name="amount" class="form-control" id="amount" step="0.01" min="0" value="{{ $repair->amount }}" />
                    </div>
                </div>
                ...
                <div class="form-group row my-2">
                    ...
                    <div class="col-sm-7">
                        <input type="date" name="repair_date" class="form-control" id="repair_date" value="{{ $repair->repair_date }}" />
                    </div>
                </div>                
                ...
                <div class="form-group row my-2">
                    ...
                    <div class="col-sm-7">
                        <select name="repair_type" class="form-control" id="repair_type">
                            <option value="repair" {{ $repair->repair_type == 'repair' ? 'selected' : '' }}>Repair</option>
                            <option value="replacement" {{ $repair->repair_type == 'replacement' ? 'selected' : '' }}>Replacement</option>
                            <option value="refill" {{ $repair->repair_type == 'refill' ? 'selected' : '' }}>Refill</option>
                        </select>
                    </div>
                </div>
